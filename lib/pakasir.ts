...
const API_KEY = process.env.PAKASIR_API_KEY || 'Sm4BXVIhCgvuTtyBLwQACJhqUdEW9QVF'
const PROJECT_SLUG = process.env.PAKASIR_PROJECT_SLUG || ''
const BASE_URL = 'https://app.pakasir.com/api'

console.log('[Kamil] Pakasir API initialized with key:', API_KEY ? '***' + API_KEY.slice(-4) : 'NOT_SET')
console.log('[Kamil] Pakasir project slug:', PROJECT_SLUG || 'NOT_SET')
...
class PakasirAPI {
  private async request<T>(
    ...
    try {
      console.log('[Kamil] Pakasir API Request:', {
        endpoint,
        method,
      ...
      console.log('[Kamil] Sending request to:', url)

      const response = await fetch(url, fetchOptions)
      const result = await response.json()
      
      console.log('[Kamil] Pakasir API Response:', {
        endpoint,
        status: response.ok,
      ...
      if (!response.ok) {
        console.error('[Kamil] Pakasir API Error Response:', result)
        return {
          status: false,
      ...
      console.error('[Kamil] Pakasir API Error:', error)
      return {
        status: false,
        code: 500,
  ...
  async getMethods(): Promise<ApiResponse<PaymentMethod[]>> {
    console.log('[Kamil] Getting Pakasir payment methods (QRIS only)')
  ...
  async createTransaction(params: {
    ...
    if (!PROJECT_SLUG) {
      console.error('[Kamil] PAKASIR_PROJECT_SLUG is not set!')
      return {
        status: false,
        code: 500,
    ...
    if (!API_KEY) {
      console.error('[Kamil] PAKASIR_API_KEY is not set!')
      return {
        status: false,
        code: 500,
    ...
    console.log('[Kamil] Creating Pakasir QRIS transaction:', {
    ...
    console.log('[Kamil] Pakasir createTransaction response received:', {
    ...
    if (!response.status || !response.data) {
      console.error('[Kamil] Failed to create Pakasir transaction:', response.message)
      return {
        status: false,
        code: response.code,
    ...
    console.log('[Kamil] Pakasir payment details:', {
    ...
    if (!pakasirData.payment.payment_number) {
      console.error('[Kamil] Pakasir response missing payment_number!', pakasirData)
      return {
        status: false,
        code: 400,
    ...
    console.log('[Kamil] Generated QR image URL for QRIS scanning')
    ...
    console.log('[Kamil] Transaction created successfully:', {
      id: transaction.id,
  ...
  async getTransactionStatus(params: {
    ...
    console.log('[Kamil] Checking Pakasir transaction status:', params.order_id)
  ...
  async cancelTransaction(params: {
    ...
    console.log('[Kamil] Canceling Pakasir transaction:', params.order_id)
  ...
  async simulatePayment(params: {
    ...
    console.log('[Kamil] Simulating Pakasir payment:', params.order_id)
