...
console.log('[Kamil] Atlantic API initialized with key:', API_KEY ? '***' + API_KEY.slice(-4) : 'NOT_SET')
...
class AtlanticAPI {
  private async request<T>(
    ...
    try {
      ...
      console.log('[Kamil] Atlantic API Request:', {
        endpoint,
      ...
      console.log('[Kamil] Request body:', formData.toString().replace(API_KEY, '***'))
      ...
      console.log('[Kamil] Atlantic API Response:', {
        endpoint,
      ...
      if (!result.status) {
        console.error('[Kamil] Atlantic API Error Response:', result)
      }
      ...
      console.error('[Kamil] Atlantic API Error:', error)
      return {
        status: false,
        code: 500,
  ...
  async getMethods(params?: {
    ...
    console.log('[Kamil] Getting payment methods with filters:', params)
  ...
  async createDeposit(
    ...
    console.log('[Kamil] Creating deposit:', {
  ...
  async getDepositStatus(id: string): Promise<ApiResponse<Transaction>> {
    console.log('[Kamil] Checking deposit status:', id)
  ...
  > {
    console.log('[Kamil] Instant deposit request:', { id, action })
  ...
  async cancelDeposit(id: string): Promise<ApiResponse<{ id: string; status: string; created_at: string }>> {
    console.log('[Kamil] Canceling deposit:', id)
  ...
  async getProfile(): Promise<ApiResponse<{
    ...
    console.log('[Kamil] Getting Atlantic profile')
