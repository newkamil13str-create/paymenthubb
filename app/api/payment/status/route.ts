...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Kamil] Checking status for transaction:', transactionData.id, 'Current status:', transactionData.status, 'Gateway:', transactionData.gateway || currentGateway)
    ...
    if (terminalStatuses.includes(transactionData.status as string)) {
      console.log('[Kamil] Transaction is in terminal state:', transactionData.status, '- returning from Redis directly')
      ...
      if (transactionData.paymentData) {
        try {
          ...
          console.error('[Kamil] Failed to parse paymentData:', e)
        }
      }
    ...
    console.log(`[Kamil] ${transactionData.gateway || currentGateway} status response:`, {
    ...
    if (statusResponse.status && statusResponse.data) {
      ...
      if (statusResponse.data.status !== transactionData.status) {
        console.log('[Kamil] Atlantic status changed from', transactionData.status, 'to', statusResponse.data.status)
        ...
        console.log('[Kamil] Status updated - Stats will be updated when webhook is received')
      }
      ...
      if (statusResponse.data.status === 'success' && !alreadyRecorded) {
        console.log('[Kamil] BACKUP: Status is success and stats not recorded - updating stats now')
        try {
          const amount = parseInt(transactionData.amount as string) || 0
          if (amount > 0) {
            // Update main stats
            ...
            console.log('[Kamil] BACKUP: Stats and leaderboard recorded successfully')
          }
        } catch (error) {
          console.error('[Kamil] BACKUP: Error updating stats:', error)
        }
      }
      ...
      if (canUseInstant && !instantDepositAlreadyTriggered) {
        console.log('[Kamil] Atlantic status is processing - AUTO-TRIGGERING instant deposit for transaction:', transactionData.id)
        try {
          ...
          console.log('[Kamil] Auto instant deposit response:', {
          ...
          if (instantResponse.status && instantResponse.data) {
            ...
            console.log('[Kamil] Auto instant deposit completed - stored metadata (instantFee:', instantResponse.data.penanganan, ', totalReceived:', instantResponse.data.total_diterima, '), awaiting webhook confirmation')
          } else {
            console.error('[Kamil] Auto instant deposit failed:', instantResponse.message)
          }
        } catch (error) {
          console.error('[Kamil] Error in auto instant deposit:', error)
          // Don't throw - just log the error and continue
        }
      } else if (canUseInstant && instantDepositAlreadyTriggered) {
        console.log('[Kamil] Instant deposit already triggered for this transaction - metadata exists')
      }
      ...
      console.log(`[Kamil] Status check complete for ${transactionData.gateway || currentGateway}. Can use instant:`, canUseInstant, '- Stats recording waits for webhook')
      ...
      console.log('[Kamil] Returning paymentData:', {
    ...
    console.error('[Kamil] Status check error:', error)
    return NextResponse.json(
