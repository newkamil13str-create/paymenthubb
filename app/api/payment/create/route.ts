...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Kamil] Payment request validated:', { amount: validated.amount, type: validated.type })
    ...
    console.log(`[Kamil] Step 1: Fetching available payment methods from ${currentGateway}...`)
    const methodsResponse = await paymentGateway.getMethods()
    console.log('[Kamil] Step 1 complete. Methods response:', { status: methodsResponse.status, count: methodsResponse.data?.length || 0, message: methodsResponse.message })
    
    if (!methodsResponse.status || !methodsResponse.data || methodsResponse.data.length === 0) {
      console.error('[Kamil] Failed to fetch payment methods:', methodsResponse.message)
      console.error('[Kamil] Full response:', methodsResponse)
    ...
    console.log('[Kamil] Available methods:', methodsResponse.data.map(m => ({ 
      metode: m.metode, 
      name: m.name, 
      type: m.type,
      status: m.status 
    ...
    if (!qrisMethod) {
      console.error(`[Kamil] No active QRIS method available in ${currentGateway} account`)
      console.error('[Kamil] Available methods:', methodsResponse.data.map(m => ({ metode: m.metode, status: m.status })))
      return NextResponse.json(
        {
          success: false,
    ...
    console.log('[Kamil] Step 2: Using QRIS method:', { metode: qrisMethod.metode, type: qrisMethod.type, name: qrisMethod.name })
    console.log(`[Kamil] Step 3: Creating deposit with ${currentGateway} API...`)
    ...
    if (!response.status || !response.data) {
      console.error('[Kamil] Step 3 failed. Response:', { status: response.status, code: response.code, message: response.message, hasData: !!response.data })
      return NextResponse.json(
    ...
    console.log('[Kamil] Step 4: Deposit created successfully. ID:', response.data.id)
    ...
    console.error('[Kamil] Payment creation error:', error)
