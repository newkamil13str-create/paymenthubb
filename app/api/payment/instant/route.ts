...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Kamil] Instant deposit request received:', {
      id: validated.id,
    ...
    if (!transactionData || !transactionData.id) {
      console.log('[Kamil] Transaction not found in Redis:', validated.id)
    ...
    console.log('[Kamil] Transaction found, current status:', transactionData.status)

    // Check if transaction is in processing status
    if (transactionData.status !== 'processing') {
      console.log('[Kamil] Transaction not in processing status:', {
    ...
    console.log('[Kamil] Atlantic instant deposit response:', {
    ...
    if (!instantResponse.status || !instantResponse.data) {
      console.error('[Kamil] Instant deposit failed:', instantResponse.message)
      return NextResponse.json(
        {
    ...
    if (validated.action) {
      console.log('[Kamil] Processing instant deposit - storing metadata')
      ...
      console.log('[Kamil] Instant deposit metadata stored - awaiting webhook confirmation')
    } else {
      console.log('[Kamil] Instant deposit fee check (preview mode)')
    }
    ...
    console.error('[Kamil] Instant deposit error:', error)
