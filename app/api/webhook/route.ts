  ...
  try {
    ...
    if (WEBHOOK_SECRET && signature) {
      ...
      if (signature !== expectedSignature) {
        console.error('[Kamil] Invalid webhook signature')
        return NextResponse.json({ error: 'Invalid signature' }, { status: 401 })
      }
    }

    const data = JSON.parse(body)
    console.log('[Kamil] Webhook received:', data)
    ...
    if (data.project && data.order_id) {
      ...
      console.log('[Kamil] Detected Pakasir webhook')
      await handlePakasirWebhook(data)
    } else if (data.event) {
      // Atlantic webhook format
      console.log('[Kamil] Detected Atlantic webhook')
      ...
      console.error('[Kamil] Unknown webhook format:', data)
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('[Kamil] Webhook error:', error)
...
async function handlePaymentSuccess(data: any) {
  ...
  console.log('[Kamil] Webhook - Processing payment.success for transaction:', transactionId)
  ...
  if (!transactionData || !transactionData.id) {
    console.error('[Kamil] Webhook - Transaction not found:', transactionId)
    return
  }
  ...
  if (alreadyRecorded) {
    console.log('[Kamil] Webhook - Stats already recorded for transaction:', transactionId, '- Skipping duplicate')
    return
  }
  ...
  try {
    ...
    console.log('[Kamil] Payment success handled and stats updated:', transactionId)
  } catch (error) {
    console.error('[Kamil] Error handling payment success:', error)
...
async function handlePakasirWebhook(data: {
  ...
  console.log('[Kamil] Processing Pakasir webhook:', data)
