...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Kamil] Leaderboard API - Redis key:', redisKey)
    ...
    const rawData = await redis.zrange(redisKey, 0, 99, {
      ...
      console.error('[Kamil] Redis zrange error:', err)
      return []
    })
    
    console.log('[Kamil] Leaderboard API - Raw data from Redis:', rawData)
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const details = await redis.hgetall(`contributor:${email}`).catch(err => {
        console.error('[Kamil] Failed to get contributor details:', err)
        return null
    ...
    console.log('[Kamil] Leaderboard API - Final response:', {
      success: true,
      period,
      type,
    ...
    console.error('[Kamil] Leaderboard API error:', error)
