...
export default function DonationsLeaderboardPage() {
  ...
  const fetchLeaderboard = async (selectedPeriod: string) => {
    try {
      ...
      console.log('[Kamil] Client - Leaderboard data received:', data)
      console.log('[Kamil] Client - Leaderboard entries:', data.leaderboard)
      console.log('[Kamil] Client - Leaderboard length:', data.leaderboard?.length)
      if (data.success) {
      ...
      console.error('[Kamil] Client - Failed to fetch leaderboard:', error)
      setLeaderboard([])
      setLoading(false)
    }
  }
